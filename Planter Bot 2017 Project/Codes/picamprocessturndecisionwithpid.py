...
rgp.setup(int(settings.get('SectionSix','LeftAPin')),rgp.OUT)
rgp.setup(int(settings.get('SectionSix','LeftBPin')),rgp.OUT)
rgp.setup(int(settings.get('SectionSix','RightAPin')),rgp.OUT)
rgp.setup(int(settings.get('SectionSix','RightBPin')),rgp.OUT)
rgp.setup(33,rgp.OUT)
rgp.setup(35,rgp.OUT)
left = rgp.PWM(33, 1000)
right = rgp.PWM(35, 1000)
left.start(50)
right.start(50)
...
class processImage(threading.Thread):
        ...
        while True:
            if(not projImages.empty()):
                ...
                if(len(self.contours) > 1):
                    ...
                    motorControlq.put((self.cx,self.midptx))
    ...
    def projection(self):
        ...
        if(np.count_nonzero(self.vectorOfZeros) > 0):
            ...
            sa3 = self.vectorOfZeros[int(settings.get('SectionFive','Zone2')):w]
            #print sa1
    ...
    def run(self):
        ...
        while True:
            if(not motorControlq.empty()):
                (self.cx,self.midx) = motorControlq.get()
                pid_controller=pc.PID(0.833,0,1)
                pid_controller.setPoint(self.midx)
                print self.cx
                pid_controller.setDerivator(self.midx-1)
                pwmDiff,self.mode = pid_controller.processPID(self.cx,self.midx)
                print abs(pwmDiff)
                if(self.mode == 0):
                    #print "Forward"
                    rgp.output(int(settings.get('SectionSix','LeftAPin')),rgp.HIGH)
                    rgp.output(int(settings.get('SectionSix','LeftBPin')),rgp.LOW)
                    rgp.output(int(settings.get('SectionSix','RightAPin')),rgp.HIGH)
                    rgp.output(int(settings.get('SectionSix','RightBPin')),rgp.LOW)
                ...
                if(self.mode==1):
                    ...
                    rgp.output(int(settings.get('SectionSix','RightAPin')),rgp.HIGH)
                    rgp.output(int(settings.get('SectionSix','RightBPin')),rgp.LOW)
                    left.ChangeDutyCycle(100-abs(pwmDiff))
                    right.ChangeDutyCycle(100)
                if(self.mode==2):
                    #print "Right"
                    rgp.output(int(settings.get('SectionSix','LeftAPin')),rgp.HIGH)
                    ...
                    rgp.output(int(settings.get('SectionSix','RightBPin')),rgp.LOW)
                    left.ChangeDutyCycle(100)
                    right.ChangeDutyCycle(100-abs(pwmDiff))
                if(self.mode==3):
                    #print "Stop"
                    rgp.output(int(settings.get('SectionSix','LeftAPin')),rgp.HIGH)
                    rgp.output(int(settings.get('SectionSix','LeftBPin')),rgp.LOW)
