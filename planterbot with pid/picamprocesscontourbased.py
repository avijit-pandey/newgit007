...
class contourProcess(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
    ...
    def run(self):
        ...
        while True:
            if(not projImages.empty()):
                ...
                c_x = int(c_moment['m10']/c_moment['m00'])
                c_y = int(c_moment['m01']/c_moment['m00'])
...
class computeProjection(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            if(not projImages.empty()):
                [(self.cx,self.x,self.w),self.im] = contourImages.get()
                ...
                if(np.count_nonzero(self.vectorOfZeros) > 0):
                    ...
                    #print idx
                    self.midx = int(settings.get('SectionOne','ResCols'))/2
                    #print self.midx
                    if(idx == 0): 
                        motorControlq.put(0)
                    if(idx == 1 and ((int(self.midx) - self.cx) > 0)):
                        motorControlq.put(1)
                    if(idx == 2 and ((int(self.midx) - self.cx) < 0)):
                        motorControlq.put(2)
                else:
                    motorControlq.put(3)
