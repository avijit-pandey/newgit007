    ...
    def run(self):
        ...
        while True:
            if(not dispImages.empty()):
                ...
                self.thresh = (gImage.max()-gImage.min()) + gImage.min()
...
class processImage(threading.Thread):
            ...
            if(not projImages.empty()):
                ...
                if(len(self.contours) >= 1):
                    ...
                    if(c_moment['m00']>0):
                        ...
                        x,y,wc,hc = cv2.boundingRect(c_max)
                #contourImages.put(([(c_x,x,w),self.im]))
                self.id = self.projection()
                #print h,w
                self.midptx = int(self.w/2)

                #print self.midpt
                
                    
                if((len(self.contours) > 1) and (c_moment['m00']>0)):
                    if((self.id == 0) and ((self.cx > (int(settings.get('SectionFive','Zone1')))) and (self.cx < (int(settings.get('SectionFive','Zone2')))))):
                        motorControlq.put(0)
                    if((self.id == 1) and ((self.cx < (int(settings.get('SectionFive','Zone1')))) and (self.cx > 0))):
                        motorControlq.put(1)
                    if((self.id == 2) and ((self.cx > (int(settings.get('SectionFive','Zone2')))) and (self.cx < self.w))):
                        motorControlq.put(2)
                else:
                    if(self.id == 0): 
                        motorControlq.put(0)
    ...
    def projection(self):
        ...
        if(np.count_nonzero(self.vectorOfZeros) > 0):
            ...
            sa3 = self.vectorOfZeros[int(settings.get('SectionFive','Zone2')):int(self.w)]
            #print sa1
